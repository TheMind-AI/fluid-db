...
class UpdateMemoryModel(BaseModel):
    ...
    data: str = Field(...,
                      description="Data that will be updated or written to the specified jsonpath. MUST be a valid json.")
...
class FetchMemoryModel(BaseModel):
    ...
    def validate_json_path_list(cls, v):
        if type(v) != list or len(v) == 0:
            raise ValueError(
                "json_path_list is not a valid list. It has to abe a list with at least one jsonpath expression")
        try:
...
class SmolMemoryAgent(DBAgent):

    def __init__(self, db_id: str):
        super().__init__()
        self.llm = OpenAILLM()
        self.db_engine = JSONEngine(db_id)
        self.db_id = db_id

    def save(self, db_id: str, messages: List[dict]):
        print("RUN, user_message:", messages)

        schema = self.db_engine.schema(db_id)
        schema_descriptions = self.db_engine.get_descriptions(db_id)
        memory = self.db_engine.get_memory(db_id)
        ...
        # First fetch data
        fetch_result = self.maybe_fetch_data(messages, memory, schema, schema_descriptions)
        print("==FETCH==")
        print(" REASONING:", fetch_result.reasoning)
        print(" JSON PATH LIST:", fetch_result.json_path_list)

        fetched_data = {json_path: self.db_engine.query(db_id, json_path) for json_path in fetch_result.json_path_list}

        llm_result = self.maybe_update_memory(messages, memory, schema, fetched_data, schema_descriptions)
        print("==UPDATE==")
        ...
        self.db_engine.update(db_id, llm_result.json_path, data, llm_result.description)
    ...
    def maybe_update_memory(self, user_message: str, memory: dict, memory_schema: str, fetched_data=None,
                            schema_descriptions: str = "") -> UpdateMemoryModel:
        prompt = self._update_memory_prompt(user_message, memory, memory_schema, fetched_data, schema_descriptions)
    ...
    def maybe_fetch_data(self, user_message: str, memory: dict, memory_schema: str,schema_descriptions: str = "") -> FetchMemoryModel:
        prompt = self._retrieve_memory_prompt(user_message, memory, memory_schema, schema_descriptions)
    ...
    @staticmethod
    def _retrieve_memory_prompt(user_message: str, memory: dict, memory_schema: str, schema_descriptions: str = ""):
        return f"""
    ...
    @staticmethod
    def _update_memory_prompt(user_message: str, memory: dict, memory_schema: str, fetched_data=None, schema_descriptions: str = ""):
        return f"""
